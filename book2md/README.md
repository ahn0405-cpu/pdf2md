...
- **이 교재의 본문 강조는 색이 아니라 「검정 굵은 글씨」다.** 171쪽을 재어
  확인했다. 청색은 제목(`2. 학설`, `IV. 시효중단`)과 두문자(`[명일동내]`)에만
  쓴다. 진짜 청색 낱말은 유채색 비율 0.88~1.00 인데, 굵은 검정은 0.08~0.28 에
  몰린다 — 굵은 획 가장자리에 JPEG 가 만든 색 번짐이다. 0.30~0.65 가 통째로
  비어 있어 그 골짜기(`min_ratio: 0.45`)에 문턱을 놓는다. **문턱을 내리면
  굵은 검정이 청색으로 둔갑해 없는 강조를 지어낸다.**

  굵기는 따로 잰다. 스캔본이라 OCR 이 붙여 준 글꼴 속성은 지어낸 것이라 못
  쓰므로, 쪽 그림에서 잰 **잉크 밀도**를 같은 줄의 가운뎃값과 견준다
  (`bold_density_ratio`). 줄이 짧으면 가운뎃값을 믿을 수 없어 판정하지 않는다.
- **강조는 낱말이 아니라 구절에 붙인다.** 낱말마다 따로 재면 획이 얇은 낱말,
  ...
  낱말별 유채색 비율, 히스토그램, 그리고 **색 화소 회계**가 나온다. 회계가
  먼저다 — 쪽 전체의 유채색 화소가 어디로 갔는지 센다.

  | 어디가 크면 | 무엇이 문제인가 | 무엇을 고치나 |
  |---|---|---|
  | `본문` 으로 판정한 낱말 안 | 색은 상자 안에 있는데 비율이 문턱을 못 넘음 | `min_ratio` 를 내린다 |
  | 어느 낱말 상자에도 없음 | **상자가 잉크와 어긋났다** | 문턱은 소용없다. 파서를 바꾼다 |
  | `강조` 가 대부분 | 판정이 맞다 | 원본에 색이 그만큼인 것이다 |

  굵기 분포도 같은 방식으로 나온다. `--image` 는 판정 결과를 쪽 그림에 네모로
  그려 준다. **초록이 색 강조, 주황이 애매, 보라가 굵은 글씨**다. 원본과
  견주면 눈으로 바로 갈린다.
