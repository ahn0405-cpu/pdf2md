...
배점은 노트에 들어가지 않는다. 쓰임은 둘뿐이다.

1. **`role` 판정.** 한 절에 사례집 문제가 여러 개 붙는다 (`V. 기판력` ← E-3,
   E-4, E-6). 노트에 사안을 다 실을 수는 없으니 어느 것이 그 절의 주된 출제
   형태인지 골라야 한다. 지침은 그 기준을 배점으로 정했다 — 최대 배점이면
   `primary`, 25% 이상이면 `composite`, 그 아래는 `incidental`(노트에 안 써도 됨).
2. **M4 검산.** 답안 배점의 합이 총점과 다르면 답안 항목을 덜 읽었다는 뜻이다.
   답안 목차는 지금 매핑의 주 근거이므로, 이 검산은 근거 자체의 건전성 검사다.

그래서 배점 숫자의 정확도보다 **어느 항목이 큰가**가 중요하다. 못 읽으면
`role` 이 `composite` 으로 떨어질 뿐 매핑이 사라지지는 않는다.
