...
- **꼬리말은 무늬가 아니라 세로 간격으로 가른다.** OCR 이 쪽마다 다르게 흘려
  놓아(`O과 nr CHAPTER 6 소송절차 개시`, `O과 己厂—I !`) 무늬로는 못 잡는다.
  각주는 줄간격으로 촘촘히 붙어 있고 꼬리말은 그 아래로 한참 떨어져 있다.
  그 간격(`running.footer_gap_ratio`)으로 가른다. 무늬(`footer_patterns`)는
  거드는 역할이다.
