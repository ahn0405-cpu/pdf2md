...
**점수를 부풀리지는 않았다.** 실물에서 사건번호는 사례집 문제 325개 중 296개가
0건이고(지침 §근거2 도 인정하는 한계다) 두문자도 절반이 0건이다. 그래서 score 2
가 드물다. 대신 같은 점수 안에서 **줄을 세운다** — 사건번호·두문자 수, 맞은
키워드의 길이 순. 우연히 짧은 키워드 하나가 겹친 것이 위로 오지 않게 한다.
