...
- **강조는 낱말이 아니라 구절에 붙인다.** 낱말마다 따로 재면 획이 얇은 낱말,
  한두 글자짜리 낱말이 문턱을 못 넘어 `==일부청구는 나머지== 부분에 대한
  ==시효중단==` 처럼 누더기가 된다. 그래서 문턱을 둘 둔다
  (`min_ratio` / `min_ratio_weak`). **확실한 낱말 사이에 낀** 애매한 낱말만
  같은 구절로 본다. 혼자 있으면 살리지 않는다 — 근거 없이 강조를 늘리면
  §5.4 대조가 통째로 무의미해진다.
  문턱은 짐작으로 정하지 말고 분포를 보고 정한다.

  ```bash
  ./convert probe 기본서.pdf --color 171
  ```

  낱말별 유채색 비율과 히스토그램이 나온다. 색칠된 낱말과 검정 낱말은 두
  봉우리로 갈리므로 **골짜기**에 문턱을 놓는다. 골짜기가 안 보이면 아직 색과
  검정이 안 갈린 것이라 `chroma_min` 을 낮추거나 `image_dpi` 를 올려야 한다.
- **머리말·꼬리말은 무늬로만 잡지 않는다.** OCR 이 쪽마다 다르게 흘려 놓아
  (`O과 nr CHAPTER 6 소송절차 개시`, `O과 己厂—I !`) 무늬로도 되풀이로도 못
  잡는 것이 있다. 세 갈래로 본다.
  1. 위·아래 띠(`repeat_zone`) 안에서 `footer_patterns` 에 맞는 줄. 이 책은
     장 제목 띠가 **위에도 아래에도** 온다.
  2. 띠 안의 **작고 짧은** 줄(`strip_small_in_band`). 본문은 그 자리에 그렇게
     서지 않는다. 제목 꼴(`046 …`, `제3편`, `IV.`)은 남긴다.
  3. 각주 영역 맨 아래에서 각주와 **세로로 뚝 떨어진** 줄
