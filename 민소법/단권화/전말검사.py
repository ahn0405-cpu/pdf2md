...
def 본다(p):
    ...
    for i, l in enumerate(L):
        ...
        if not re.search(r"[甲乙丙丁戊ABKXY]", t):
            무인물.append(i + 1)
        # 앞 판례의 상황을 이어받고 새 내용이 전부 소송 경위뿐이면 합쳐도 된다.
        # 나누면 「상황: 위와 같다」만 남는 빈 항목이 생긴다 (샘플 5곳이 그렇다).
        이어받음 = re.search(
            r"같은 ?\*{0,2}(사안|국면)|위와 같|위 사례와 같|사례 그대로|사안에서 이어|사안 그대로", t)
        if "**상황과 주장**" in t and not 이어받음:
            안나뉨.append(i + 1)
    return 없음, 무인물, 안나뉨
